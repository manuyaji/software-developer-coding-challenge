...
public class ErrorProperties {
	...
	public ResponseError getResponseErrorDetails(ErrorCode errorCode, Object ...params){
		try{
			int severity = Integer.parseInt(resourceBundle.getString(String.format(SEVERITY_FORMAT, errorCode)));
			String shortMessage = resourceBundle.getString(String.format(SHORT_MESSAGE_FORMAT, errorCode));
			String description = buildDescription(resourceBundle.getString(String.format(DESCRIPTION_FORMAT,errorCode)), params);
			return ResponseError.builder().errorCode(errorCode).severity(severity).shortMessage(shortMessage).description(description).build();
